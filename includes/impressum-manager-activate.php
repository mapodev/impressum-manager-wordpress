...
function impressum_manager_insert_data() {
	...
	$impressum_key['address_telephone'] = $_format_contact_telephone = __("<tr><td>Telefon:</td><td>%%PHONE%%</td></tr>", $domain);
	$impressum_key['address_telefax'] = $_format_contact_telefax = __("<tr><td>Telefax:</td><td>%%FAX%%</td></tr>", $domain);
	$impressum_key['address_email'] = $_format_contact_email = __("<tr><td>E-Mail:</td><td>%%EMAIL%%</td></tr>", $domain);
	$impressum_key['register_header'] = $_format_register = __("<h2>Registereintrag:</h2>", $domain);
	$impressum_key['register_entry'] = $_format_register_registered_in = __("Eintragung im %%REGISTER%%.", $domain);
	$impressum_key['register_number'] = $_format_register_registernr = __("<br>Registernummer: %%REGISTERNUMBER%%", $domain);
	$impressum_key['register_chamber'] = $_format_register_chamber = __("<br>Registergericht: %%CHMABER%%", $domain);
	$impressum_key['vat'] = $_format_vat = __("<h2>Umsatzsteuer-ID:</h2><p>Umsatzsteuer-Identifikationsnummer gemäß §27 a Umsatzsteuergesetz:<br>%%VAT%%</p>", $domain);
	$impressum_key['control_chamber'] = $_format_office = __("<h2>Aufsichtsbehörde:</h2><p>%%RESPONSIBLECHAMBER%%</p><p>Berufsbezeichnung: %%REGULATEDPROFESSION%%<br>Zuständige Kammer: %%CHAMBER%%<br>Verliehen durch: %%STATE%%<br>Es gelten folgende berufsrechtliche Regelungen: %%STATERULES%%<br></p>", $domain);
