...
class WPImpressum
{
    ...
    {
        self::$_format_address = __("<h2>Angaben gemäß § 5 TMG:</h2>");
        self::$_format_representant = __("<h2>Vertreten durch:</h2><p>[Vertreten durch: %s, %s]</p>");
        self::$_format_contact = __("<h2>Kontakt:</h2>");
        self::$_format_contact_telephone = __("<tr><td>Telefon:</td><td>%s</td></tr>");
        self::$_format_contact_telefax = __("<tr><td>Telefax:</td><td>%s</td></tr>");
        self::$_format_contact_email = __("<tr><td>E-Mail:</td><td>%s</td></tr>");
        self::$_format_register = __("<h2>Registereintrag:</h2>");
        self::$_format_register_registered_in = __("Eintragung im %s.");
        self::$_format_register_registernr = __("<br>Registernummer: %s");
        self::$_format_register_chamber = __("<br>Registergericht: %s");
        self::$_format_vat = __("<h2>Umsatzsteuer-ID:</h2><p>Umsatzsteuer-Identifikationsnummer gemäß §27 a Umsatzsteuergesetz:<br>%s</p>");
        self::$_format_office = __("<h2>Aufsichtsbehörde:</h2><p>Aufsichtsbehörde</p><p>Berufsbezeichnung: gesetzl. Berufsbezeichnung:<br>Zuständige Kammer: %s<br>Verliehen durch: %s<br>Es gelten folgende berufsrechtliche Regelungen: %s<br></p>");
        ...
        self::$_privacy_policy_head = __("<h2>Datenschutzerklärung:</h2>");
        ...
        self::$_privacy_policy_end = __("<strong>Auskunft, L&ouml;schung, Sperrung</strong></p> <p>Sie haben jederzeit das Recht auf unentgeltliche Auskunft &uuml;ber Ihre gespeicherten personenbezogenen Daten, deren Herkunft und Empf&auml;nger und den Zweck der Datenverarbeitung sowie ein Recht auf Berichtigung, Sperrung oder L&ouml;schung dieser Daten. Hierzu sowie zu weiteren Fragen zum Thema personenbezogene Daten k&ouml;nnen Sie sich jederzeit &uuml;ber die im Impressum angegeben Adresse des Webseitenbetreibers an uns wenden.</p>");
        self::$_source = __("<p>Quelle: <em><a rel=\"nofollow\" href=\"http://www.e-recht24.de/impressum-generator.html\">http://www.e-recht24.de</a></em></p>");
        self::$_plugin_by = __("<p>Plugin von <a href=\"http://www.wp-impressum.com\">WP-Impressum</a></p>");
    }
    ...
    {
        return sprintf(__(self::$_format_representant), $name, $address);
    }
    ...
    private function wpimpressum_return_register($lang, $register_chamber, $registernr, $register)
    {
        switch ($register) {
            case 1:
                $register_registered_in = __("Kein Register");
                break;
            case 2:
                $register_registered_in = __("Genossenschaftsregister");
                break;
            case 3:
                $register_registered_in = __("Handelsregister");
                break;
            case 4:
                $register_registered_in = __("Partnerschaftsregister");
                break;
            case 5:
                $register_registered_in = __("Vereinsregister");
                break;
            default:
                $register_registered_in = __("Kein Register");
                break;
        ...
        $result = __(self::$_format_register);
        $result .= "<p>";

        if (!empty($register)) $result .= sprintf(__(self::$_format_register_registered_in), $register_registered_in);
        if (!empty($registernr)) $result .= sprintf(__(self::$_format_register_registernr), $registernr);
        if (!empty($register_chamber)) $result .= sprintf(__(self::$_format_register_chamber), $register_chamber);
        $result .= "</p>";
        return $result;
    }
    ...
    {
        if (!empty($vat)) return sprintf(__(self::$_format_vat), $vat);
        return "";
    }

    private function wpimpressum_return_privacy_policy($lang, $general, $facebook, $analytics, $adsense, $plus, $twitter)
    {
        $result = __(self::$_privacy_policy_head);
        if (strlen($general) > 0) $result .= __(self::$_privacy_policy_general);
        if (strlen($facebook) > 0) $result .= __(self::$_privacy_policy_facebook);
        if (strlen($analytics) > 0) $result .= __(self::$_privacy_policy_analytics);
        if (strlen($adsense) > 0) $result .= __(self::$_privacy_policy_adsense);
        if (strlen($plus) > 0) $result .= __(self::$_privacy_policy_plus);
        if (strlen($twitter) > 0) $result .= __(self::$_privacy_policy_twitter);
        $result .= __(self::$_privacy_policy_end);
        return $result;
    }
}
